...
void main()
{
   ...
   vec3 specular = uLight.specular * spec * vec3(texture(uMaterial.specular, varingTexCoords));

   // 4. Emission
   vec3 emiision = texture(uMaterial.emission, varingTexCoords).rgb;

   vec3 result = (ambient + diffuse + specular + emiision);
