...
void ProcessInput(GLFWwindow* window)
{
    ...
    {
        TextureMixAlpha += 0.001f;
        if (TextureMixAlpha > 1.0f)
            TextureMixAlpha = 1.0f;
    }
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
    {
        TextureMixAlpha -= 0.001f;
        if (TextureMixAlpha < 0.0f)
            TextureMixAlpha = 0.0f;
    }
}
...
int main(int argc, char** argv)
{
    ...
    Shader shader("./src/04-Textures-practice4/Shaders/VertexShader.glsl", "./src/04-Textures-practice4/Shaders/FragmentShader.glsl");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shader.Use();
        shader.SetFloat("uMixAlpha", TextureMixAlpha);
        glBindVertexArray(vao);
